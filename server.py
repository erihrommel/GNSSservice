    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.25o') as f:
            ...
            while total < file_size:
                ...
                f.write(chunk)
                total += len(chunk)
            print(f"[Сервер] Сохранен BASE файл: {base_filename} -> {base_path}")

        name_len1 = struct.unpack('>I', recv_exactly(conn, 4))[0]
        rover_filename = recv_exactly(conn, name_len1).decode('utf-8')
        file_size1 = struct.unpack('>Q', recv_exactly(conn, 8))[0]

        # Сохраняем RINEX во временный файл
        with tempfile.NamedTemporaryFile(delete=False, suffix='.25o') as f1:
            rover_path = f1.name
            ...
            while total < file_size1:
                ...
                f1.write(chunk)
                total += len(chunk)
            print(f"[Сервер] Сохранен ROVER файл: {rover_filename} -> {rover_path}")
        ...
        eph_path = download_brdc_from_bkg(obs_date, out_dir=os.path.dirname(base_path))
        print(f"[Сервер] Скачаны эфемериды: {eph_path}")

        # Запускаем RTKLIB напрямую
        result_path = base_path.replace('.obs', '.pos')
